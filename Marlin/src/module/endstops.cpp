...
void Endstops::update() {
  ...
  #if ENABLED NON_E_AXES > 3
    #define I_STOP (I_HOME_DIR < 0 ? I_MIN : I_MAX)
    #if ENABLED NON_E_AXES > 4
      #define K_STOP (J_HOME_DIR < 0 ? J_MIN : J_MAX)
      #if ENABLED NON_E_AXES > 5
        #define K_STOP (K_HOME_DIR < 0 ? K_MIN : K_MAX)
      #endif
    #endif
  #endif
  ...
  void Endstops::clear_endstop_state() {
    ...
    TERN_(I_SPI_SENSORLESS, CBI(live_state, I_STOP));
    TERN_(J_SPI_SENSORLESS, CBI(live_state, J_STOP));
    TERN_(K_SPI_SENSORLESS, CBI(live_state, K_STOP));
  }
